...
NULL CLAW는 외부 API(OpenAI, Claude 등)나 무거운 로컬 LLM을 전혀 사용하지 않고, 오직 파이썬 내장 라이브러리와 자동화 패키지만을 활용하여 PC를 제어합니다. 
...
- **Global CLI:** 어디서든 `null-claw` 명령어로 실행 가능.
...
```bash
# 1. 레포지토리 클론
git clone https://github.com/manus-ai/null-claw.git
cd null-claw
...
`src/null_claw/plugins/` 디렉토리에 새로운 파일을 만들고 `BasePlugin`을 상속받아 구현하세요.
